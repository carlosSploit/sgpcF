...
export async function getInformationAmenaz({id_activProsVerAnali = 0, id_afectaActiv = 0}) {
  const url = `${domain_api}/planesconting/inforamenaz/${id_activProsVerAnali}/${id_afectaActiv}`;
  const result = await axios.get(url, {
    headers: {
      Authorization: `Bearer ${await gettoken()}`,
    },
