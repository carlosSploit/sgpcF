...
import { Itemcontenresul } from "../itemcontenresul/itemcontenresul";
...
export function Conviewtarea(props){
    ...
    const onListResultTarea = async () => {
        ...
        let result = await getResulTarea(dataac.id_conte_tarea);
        // console.log(result);
            ...
            <ComponentModalPrincipal  statemode={ismodalvisible} onClosechange={()=>{setismodalvisible(false);}}>
                ...
                <ComponentModalPrincipalBody>
                    ...
                    <div className="container_view_resulttar_contain_info_contain_title">
                        <div className="container_view_resulttar_contain_info_title">Resoluciones</div>
                    </div>
                    <div style={{height: "10px"}} />
                    {
                        (listresulttarea != null)?
                        listresulttarea.map((item)=>{
                            return <Itemcontenresul datatart={dataac} onClickActions={onListResultTarea} dataac={item} label={item.nombre}></Itemcontenresul>
                        }):<></>
                    }
                </ComponentModalPrincipalBody>
